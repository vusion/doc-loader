...
module.exports = function (content) {
    ...
    componentRoute.children = components.filter((component) => !!component.path).map((component) => ({ path: component.name, fullPath: component.path }));
    components.forEach((component) => {
        if (component.testPath)
            componentRoute.children.push({ path: component.name + '~test', fullPath: component.testPath });
    });
