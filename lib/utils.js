...
exports.getMaterials = function (basePath, materials, type) {
    if (!materials && type === 'components') { // 如果配置中没有 materials，则根据目录来生成
        const materialsMap = {};
    ...
    materials.forEach((material) => {
        ...
        if (material.path) {
            ...
            if (path.isAbsolute(material.path) && !fs.existsSync(material.path)) {
                ...
                const depMarkdownPath = path.resolve(process.cwd(), 'node_modules/proto-ui.vusion/src/' + material.name + (type === 'components' ? '.vue/README.md' : '/README.md')).replace(/\\/g, '/');
