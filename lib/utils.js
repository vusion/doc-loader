...
exports.nestRoutes = function (flatRoutes) {
    ...
    const parse = function (route) {
        ...
        if (!parent) {
            ...
            parent.fullPath = parent.filePath = path.resolve(__dirname, '../layouts/wrapper.vue').replace(/\\/g, '/');
            parse(parent);
        }
...
exports.getComponents = function (basePath, components) {
    ...
    components.forEach((component) => {
        if (component.path) {
            if (component.path[0] === '.')
                component.path = path.join(process.cwd(), component.path).replace(/\\/g, '/');
            else {
                // @compat:
                let libraryPath = basePath;
                if (basePath.endsWith('components'))
                    libraryPath = path.dirname(basePath);
                component.path = component.path.replace(/^library/, libraryPath).replace(/^@/, process.cwd());
            }
        } else if (!component.href && !component.to)
            component.path = path.resolve(basePath, component.name + '.vue/README.md').replace(/\\/g, '/');

        if (component.path) {
            // 只验证完整路径
            if (path.isAbsolute(component.path) && !fs.existsSync(component.path)) {
                component.path = '';
            ...
            // subDocs
            const subDocsPath = path.resolve(component.path, '../docs');
            if (fs.existsSync(subDocsPath)) {
                component.children = fs.readdirSync(subDocsPath).map((name) => ({
                    path: name.replace(/\.md$/, ''),
                    fullPath: path.resolve(subDocsPath, name),
                }));
                if (component.children && component.children[0] && !!component.children[0].path) {
                    if (component.children.find((route) => route.path === 'examples'))
                        component.children.unshift({ path: '', redirect: 'examples' });
                    else
                        component.children.unshift({ path: '', redirect: component.children[0].path });
                }
            }
        }
