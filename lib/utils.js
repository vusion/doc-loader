...
function ensureReadmePath(markdownPath) {
    if (!markdownPath.endsWith('.vue/README.md'))
        return markdownPath;

    const yamlPath = path.join(markdownPath, '../api.yaml');
    ...
    materials.forEach((material) => {
        ...
        if (material.path) {
            ...
            if (path.isAbsolute(material.path) && !fs.existsSync(material.path)) {
                ...
                let depMarkdownPath;
                if (type !== 'vendors')
                    depMarkdownPath = ensureReadmePath(path.resolve(basePath, `../../node_modules/proto-ui.vusion/src/${type}/` + relativeReadmePath).replace(/\\/g, '/'));
                else
                    depMarkdownPath = ensureReadmePath(path.resolve(basePath, `../../node_modules/proto-ui.vusion/node_modules/` + relativeReadmePath).replace(/\\/g, '/'));
            ...
            // subDocs
            let subDocsPath;
            if (path.isAbsolute(material.path) || material.path[0] === '.')
                subDocsPath = path.resolve(material.path, '../docs');
            else
                subDocsPath = path.resolve('node_modules', material.path, '../docs');

            const subDocsExists = fs.existsSync(subDocsPath);
            const pathIsYaml = material.path.endsWith('.vue/api.yaml');
